...
namespace FolderSynchronisationApp
{
    internal class ServiceRepository : IServiceRepository
    {
        ...
        public ServiceRepository(IHostApplicationLifetime lifeTime, IOptions<SettingConfig> settingConfig)
        {
            _lifeTime = lifeTime;
        ...
        {
            string[] sourceFilesComb = sourceFiles.Select(f => Path.GetFileName(f)).ToArray();
            string[] replicaFilesComb = replicaFiles.Select(f => Path.GetFileName(f)).ToArray();

            string[] filesToDelete = replicaFilesComb.Except(sourceFilesComb).ToArray();
